  ...
  return (
    <div>
      ...
      <form onSubmit={createWidget}>
        ...
        <input placeholder="Price (cents)" value={form.price_cents} onChange={(e) => setForm({ ...form, price_cents: e.target.value })} required />
        <input placeholder="Stock quantity" value={form.stock_quantity} onChange={(e) => setForm({ ...form, stock_quantity: e.target.value })} />
        ...
        <tbody>
          {widgets.map((w) => (
            <tr key={w.id}>
              ...
              <td>
                <button onClick={() => updatePrice(w)}>Set price</button>
              </td>
            </tr>
          ))}
        </tbody>
